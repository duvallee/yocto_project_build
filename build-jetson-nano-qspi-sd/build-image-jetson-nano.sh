...
export BUILD_IMAGES="image-jetson-nano-jetson-nano-qspi-sd.tegraflash.tar.gz"
...
# MAIN LOOP
{
  ...
  if [ "${1}" != "-u" ]
   then
     bitbake image-jetson-nano -c clean
   fi

   bitbake image-jetson-nano
   build_end_time
   # ------------------------------------------------------------------
   ...
   cd ../
   mv ./tmp/image-jetson-nano.sdcard image-jetson-nano.sdcard.img
   rm -rf ./tmp
